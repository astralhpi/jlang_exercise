...
def fact(*_coconut_match_to_args, **_coconut_match_to_kwargs):
    ...
    if not _coconut_match_check:
        _coconut_match_err = _coconut_MatchError("pattern-matching failed for " "'def fact(1) = 1'" " in " + _coconut.repr(_coconut.repr(_coconut_match_to_args)))
        _coconut_match_err.pattern = 'def fact(1) = 1'
...
def fact(*_coconut_match_to_args, **_coconut_match_to_kwargs):
    ...
    if _coconut_match_check and not (n > 1):
        _coconut_match_check = False
    if not _coconut_match_check:
        _coconut_match_err = _coconut_MatchError("pattern-matching failed for " "'def fact(n is int if n > 1) = fact(n-1) * n'" " in " + _coconut.repr(_coconut.repr(_coconut_match_to_args)))
        _coconut_match_err.pattern = 'def fact(n is int if n > 1) = fact(n-1) * n'
        _coconut_match_err.value = _coconut_match_to_args
        raise _coconut_match_err
